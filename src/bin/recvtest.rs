...
fn main() {
            ...
            Ok((size, _)) => {
                ...
                if count > 0 {
                    ...
                    if count <= 10 || count % 20 == 0 {
                        println!("Packet {}: size={}, delta={:.3}ms",
                                 count, size, delta as f64 / 1_000_000.0);
                    }
                }
                last_recv = now;
                count += 1;
    ...
    if deltas.len() > 10 {
        ...
        println!("Min inter-packet time:    {:.3} ms", min as f64 / 1_000_000.0);
        println!("Max inter-packet time:    {:.3} ms", max as f64 / 1_000_000.0);
        println!("Median inter-packet time: {:.3} ms", median as f64 / 1_000_000.0);
        println!("Mean inter-packet time:   {:.3} ms", mean as f64 / 1_000_000.0);
        println!("Std deviation:            {:.3} ms ({:.1} us)", std_dev / 1_000_000.0, std_dev / 1000.0);

        // This std dev represents the jitter in packet arrival timing
        println!("\n>>> Packet arrival jitter (std dev): {:.1} us", std_dev / 1000.0);
    }
