...
use dantesync::date_offset::{
    DateAnnounce, DateAuthority, DateFollower, FollowAction, StepKind, DEFAULT_STEP_BOUND_NS,
    MIN_STEP_LEAD_NS,
};
...
struct Box_ {
    ...
    /// The grandmaster this box currently hears (switches a few windows after the real change).
    seen_gm: u8,
    /// The grandmaster the phase-lock anchor belongs to (the controller keeps `current_gm_uuid`).
    core_gm: u8,
    gm_switch_lag_windows: u64,
    rng: Rng,
    // metrics
    words: Vec<f64>,
...
fn run(utc_vs_gm_ppm: f64) -> RunResult {
    ...
    let mut boxes: Vec<Box_> = (0..oscs.len())
        .map(|i| Box_ {
            ...
            seen_gm: 1,
            core_gm: 1,
            gm_switch_lag_windows: (i as u64 * 3) % 7,
    ...
    for w in 0..windows {
        ...
        gm_a.time.advance(true_dt_ns, gm_a.rate_ppm);
        gm_b.time.advance(true_dt_ns, gm_b.rate_ppm);
        let current_gm = if w >= GM_CHANGE_AT_WINDOW { 2 } else { 1 };
        for b in boxes.iter_mut() {
            ...
            match crossing {
                Some((p, to_cross)) => {
                    ...
                    b.stepped += due.delta_ns;
                    b.core.note_step(due.delta_ns);
                    b.steps.push((
                        due.seq,
                        due.delta_ns,
                ...
                None => b.wall.advance(true_dt_ns, rate),
            }
            if current_gm == 2 && w >= GM_CHANGE_AT_WINDOW + b.gm_switch_lag_windows {
                b.seen_gm = 2;
            }
        }
        ...
        for b in boxes.iter_mut() {
            if let Some(due) = b.follower.due(b.wall_ns()) {
                ...
                b.core.note_step(due.delta_ns);
                b.steps
                    .push((due.seq, due.delta_ns, StepKind::Late, t0_ns + true_dt_ns));
            }
        }
        ...
        for (i, b) in boxes.iter_mut().enumerate() {
            let gm = if b.seen_gm == 1 { &gm_a } else { &gm_b };
            let mut samples: Vec<i64> = (0..SAMPLES_PER_WINDOW)
                .map(|_| {
                    ...
                    b.wall_ns() - gm.time.ns + (b.delay_ns + noise).round() as i64
                })
                .collect();
            samples.sort();
            let median = samples[SAMPLES_PER_WINDOW / 2];
            if gm.id != b.core_gm {
                // The controller's GRANDMASTER UUID CHANGED path.
        ...
        // 4. the master: authority lifecycle + UTC every 10 s.
        {
            let m = &mut boxes[0];
            let anchor = m.core.anchor_ns().unwrap();
            let now_ptp = m.wall_ns() - anchor;
            if authority.is_none() {
                authority = Some(DateAuthority::new(
                    anchor,
                    now_ptp,
        ...
        if w % POLL_INTERVAL_WINDOWS == 0 {
            ...
            for b in boxes.iter_mut().skip(1) {
                ...
                if b.core_gm != master_gm {
                    continue; // D belongs to another PTP time base: never adopt it
                }
                ...
                match b.follower.on_announce(ann, anchor, b.wall_ns()) {
                    ...
                    FollowAction::Step { delta_ns, kind } => {
                        b.stepped += delta_ns;
                        b.core.note_step(delta_ns);
                        b.steps.push((ann.seq, delta_ns, kind, t0_ns + true_dt_ns));
                    }
                }
            }
        }
        ...
        if w % 7_200 == 0 {
            // hourly effective rate vs the grandmaster each box currently hears (skip the hour
            // that contains the GM change: its window mixes two rates by construction)
            let gm_ns = |id: u8| if id == 1 { gm_a.time.ns } else { gm_b.time.ns };
            for (i, b) in boxes.iter().enumerate() {
                let (w0, g0) = hour_start[i];
                if w > 7_200 && !(w - 7_200..w).contains(&GM_CHANGE_AT_WINDOW) {
                    // The continuous clock (the wall without its steps) against the GM's time.
                    let d_wall = (b.wall.ns - w0) as f64;
                    let d_gm = (gm_ns(b.seen_gm) - g0) as f64;
                    rate_errors.push((d_wall / d_gm - 1.0) * 1e6);
                }
                hour_start[i] = (b.wall.ns, gm_ns(b.seen_gm));
            }
        }
    }
...
fn check(label: &str, r: &RunResult) {
    ...
    // Every box re-anchored exactly once — at the grandmaster change — and never stepped for it.
    assert!(
        r.rebases.iter().all(|&x| x == 1),
        "[{label}] rebases {:?}",
        r.rebases
    );
...
fn two_clock_bench_rate_is_ptp_phase_agrees_and_every_date_step_is_coordinated_117_88() {
    ...
    // THE DECOUPLING STATEMENT: the two runs share every PTP input and differ ONLY in UTC (so in
    // the number, size and timing of the date steps). The frequency command of every box is
    // bit-identical between them: NTP contributes nothing to the rate.
